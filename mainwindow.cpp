...
//     setWindowTitle("Object Distance Detection");
//     resize(1024, 768);
...
//                 detection.center = cv::Point(centerX, centerY);
...
//     // Create a vector of filtered detections after NMS
//     std::vector<Detection> filteredDetections;
//     for (size_t i = 0; i < indices.size(); ++i) {
//         filteredDetections.push_back(detections[indices[i]]);
//     }

//     // Calibration mode - look for cell phone or other reference object
//     if (calibrationMode && !filteredDetections.empty()) {
//         for (const auto& detection : filteredDetections) {
//             // If we detect a cell phone as reference object
//             if (detection.className == "cell phone") {
//                 // Use the width of the detected reference object to calibrate
//                 pixelsPerCm = calibratePixelsPerCm(detection.box.width, referenceWidthCm);
//                 isCalibrated = true;
//                 calibrationMode = false;
//                 std::cout << "Calibrated: " << pixelsPerCm << " pixels per cm" << std::endl;
//                 break;
//             }
//         }
//     }
...
//         drawPred(filteredDetections[i], frame);

//         // Calculate distances between this object and all other objects
//         for (size_t j = i + 1; j < filteredDetections.size(); ++j) {
//             // Calculate pixel distance between centers
//             double pixelDist = calculatePixelDistance(
//                 filteredDetections[i].center,
//                 filteredDetections[j].center
//                 );

//             // Convert to real-world distance if calibrated
//             double realDist = isCalibrated ? pixelToRealDistance(pixelDist, pixelsPerCm) : -1;

//             // Draw a line between the two objects
//             cv::line(frame, filteredDetections[i].center, filteredDetections[j].center,
//                      cv::Scalar(0, 255, 255), 2);

//             // Calculate the midpoint to display the distance
//             cv::Point midPoint = (filteredDetections[i].center + filteredDetections[j].center) / 2;

//             // Display the distances
//             std::string distText;
//             if (isCalibrated) {
//                 distText = cv::format("Px: %.1f, Dist: %.1f cm", pixelDist, realDist);
//             } else {
//                 distText = cv::format("Px: %.1f (Not calibrated)", pixelDist);
//             }
//             cv::putText(frame, distText, midPoint,
...
//     if (calibrationMode) {
//         calibrationInfo = "Calibration mode: Please show a cell phone";
//     } else if (isCalibrated) {
//         calibrationInfo = "Calibrated: " + std::to_string(pixelsPerCm) + " pixels/cm";
//     } else {
//         calibrationInfo = "Not calibrated";
//     }
//     cv::putText(frame, calibrationInfo, cv::Point(10, 30),
//                 cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 0, 0), 2);

//     // Display information about the number of objects detected
//     std::string objectsInfo = "Objects detected: " + std::to_string(filteredDetections.size());
//     cv::putText(frame, objectsInfo, cv::Point(10, 60),
//                 cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 0, 0), 2);
...
// double MainWindow::pixelToRealDistance(double pixelDistance, double pixelsPerCm) {
//     return pixelDistance / pixelsPerCm;
// }

// double MainWindow::calibratePixelsPerCm(int referenceWidthPixels, double referenceWidthCm) {
//     return static_cast<double>(referenceWidthPixels) / referenceWidthCm;
// }
#include "mainwindow.h"
#include <QVBoxLayout>
#include <QHBoxLayout>
